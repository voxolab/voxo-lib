...
class ResilientSession(Session):
    ...
    def __init__(self, number_of_retries = 500, delay = 10):
        super().__init__()
    ...
    def __recoverable(self, error, url, request, counter=1):
        ...
        if not isinstance(error, str):
            if hasattr(error,'status_code'):
                if error.status_code in [200, 404, 401]:
                    return False

        logging.warn("Got recoverable error [%s] from %s %s, retry #%s/%s in %ss" % (error, request, url, counter, self.number_of_retries, self.delay))
        time.sleep(self.delay)

        if((self.number_of_retries - counter) < 0):
            return False
        else:
            return True
